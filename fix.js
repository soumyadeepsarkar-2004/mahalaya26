...
const startStr = '<!-- DESKTOP RADIO CARD -->';
const startStr2 = '<!-- RADIO DESKTOP -->';
const endStr = '<!-- DESKTOP UTILITIES -->';

const start = content.indexOf(startStr) !== -1 ? content.indexOf(startStr) : content.indexOf(startStr2);
const end = content.indexOf(endStr);

if (start !== -1 && end !== -1) {
    const radioHtml = `        <!-- RADIO DESKTOP -->
...
`;
    content = content.substring(0, start) + radioHtml + content.substring(end);
    fs.writeFileSync('index.html', content);
}

// Now replace #sheet-radio
let startSheet = content.indexOf('<!-- Mobile Radio Expanded Sheet -->');
let endSheet = content.indexOf('<!-- Mobile Status Sheet (Time/Countdown) -->');
if(startSheet !== -1 && endSheet !== -1) {
    const sheetHtml = `<!-- Mobile Radio Expanded Sheet -->
    <div id="sheet-radio" class="fixed bottom-0 left-0 w-full rounded-t-[4px] p-6 pb-[calc(24px+env(safe-area-inset-bottom,0px))] z-50 transform translate-y-full transition-transform duration-400 ease-[cubic-bezier(0.32,0.72,0,1)] pointer-events-auto flex flex-col items-center border-t border-white/10" style="background: rgba(var(--scene-rgb), 0.15); backdrop-filter: blur(32px); -webkit-backdrop-filter: blur(32px);">
        <div class="w-10 h-1 bg-white/20 mb-8 sheet-handle"></div>
        
        <div class="w-full max-w-[240px] aspect-square bg-cover bg-center border border-white/10 mb-8 shadow-2xl transition-all duration-700" id="radio-thumb-mobile"></div>
        
        <span class="text-[9px] font-mono tracking-[0.3em] text-white/50 mb-2">PUJA RADIO</span>
        <span class="font-serif text-2xl text-white/90 text-center px-4 mb-1" id="player-track-sheet">...</span>
        
        <div class="w-12 h-px bg-white/10 my-6"></div>
        
        <div class="flex items-center gap-10">
            <button id="btn-prev-mobile" class="text-white/40 hover:text-white outline-none"><i class="fas fa-backward"></i></button>
            <button id="btn-play-pause-sheet" class="w-16 h-16 rounded-full border border-white/20 flex items-center justify-center outline-none">
                <i class="fas fa-play text-lg text-white translate-x-[2px]" id="play-icon-sheet"></i>
            </button>
            <button id="btn-next-mobile" class="text-white/40 hover:text-white outline-none"><i class="fas fa-forward"></i></button>
        </div>
        
        <div class="w-full max-w-[240px] mt-8 flex items-center gap-3">
            <i class="fas fa-volume-down text-[10px] text-white/30"></i>
            <input type="range" id="volume-slider-mobile" min="0" max="1" step="0.01" value="0.8" class="w-full h-[2px] bg-white/20 appearance-none slider-thumb-analog">
            <i class="fas fa-volume-up text-[10px] text-white/30"></i>
        </div>
    </div>

    `;
    content = content.substring(0, startSheet) + sheetHtml + content.substring(endSheet);
    fs.writeFileSync('index.html', content);
}
